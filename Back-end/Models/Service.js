// Back-end/Models/Service.js - مُصحح مع دعم الصور
...
serviceSchema.pre('deleteOne', { document: true, query: false }, async function(next) {
  try {
    ...
    // 🆕 حذف صورة الخدمة من Cloudinary إذا كانت موجودة
    if (this.image) {
      const { deleteFromCloudinary } = require('../config/cloudinary');
      await deleteFromCloudinary(this.image);
      console.log(`Deleted service image: ${this.image}`);
    }
