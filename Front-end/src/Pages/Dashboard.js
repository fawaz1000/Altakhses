...
export default function Dashboard() {
  ...
  const handleAddPartner = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${API_BASE}/api/partners`, formData, {
        headers: {
      ...
      const errorMessage =
        error.response?.data?.message || "فشل في إضافة الشريك";
  ...
  const handleUpdatePartner = async (id) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "فشل في تحديث بيانات الشريك";
      showMessage("error", errorMessage);
    }
  };
  ...
  const handleTogglePartnerStatus = async (id) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "فشل في تغيير حالة الشريك";
      showMessage("error", errorMessage);
    }
  };
  ...
  const handleAddBranch = async (e) => {
    ...
    try {
      const { data } = await axios.post(`${API_BASE}/api/branches`, newBranch, {
        headers: {
      ...
      const errorMessage =
        error.response?.data?.message || "فشل في إضافة الفرع";
      showMessage("error", errorMessage);
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleUpdateBranch = async (id) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "فشل في تحديث بيانات الفرع";
      showMessage("error", errorMessage);
    }
  };
    ...
    try {
      ...
      if (data.success) {
        ...
        window.dispatchEvent(
          new CustomEvent("logoUpdated", { detail: data.data.logo })
        );
      }
    } catch (error) {
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || error.message || "فشل في إضافة القسم";
      showMessage("error", errorMessage);
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleUpdateCategory = async (id) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || error.message || "فشل في تحديث القسم";
      showMessage("error", errorMessage);
    }
  };
  ...
  const handleDeleteCategory = async (id) => {
    if (
      !window.confirm(
        "هل أنت متأكد من حذف هذا القسم؟ سيتم حذف جميع الخدمات المرتبطة به أيضاً."
      )
    )
      return;
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.error || error.message || "فشل في حذف القسم";
      showMessage("error", errorMessage);
    }
  };
  ...
  const handleAddService = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${API_BASE}/api/services`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleUpdateService = async (id) => {
    ...
    try {
      ...
      if (updatedService.categoryId)
        formData.append("categoryId", updatedService.categoryId);
  ...
  const handleAddDoctor = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${API_BASE}/api/doctors`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",
          Authorization: `Bearer ${token}`,
        },
      ...
      const errorMessage =
        error.response?.data?.message || "فشل في إضافة الطبيب";
      showMessage("error", errorMessage);
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleAdd = async (e) => {
    ...
    // التحقق من وجود ملف
    if (!newMedia.file) {
      showMessage("error", "يرجى اختيار ملف للرفع");
      return;
    }
  ...
  const handleSaveReports = async (e) => {
    ...
    const emptyFields = requiredFields.filter(
      (field) => !newMetric[field] || newMetric[field] === ""
    );
    ...
    const metrics = [
      ...
      {
        label: "حالات معالجة",
        count: parseInt(newMetric.operations),
        suffix: "+",
      },
      {
        label: "عدد المراجعين",
        count: parseInt(newMetric.visitors),
        suffix: "+",
      },
      {
        label: "عدد المراجعين الجدد",
        count: parseInt(newMetric.newVisitors),
        suffix: "+",
      },
    ];
    ...
    try {
      ...
      showMessage(
        "success",
        `تم حفظ تقرير سنة ${parseInt(newMetric.year)} بنجاح`
      );
    } catch (error) {
      const errorMessage =
        error.response?.data?.message || "فشل في حفظ التقرير";
      showMessage("error", errorMessage);
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      <aside className="w-72 bg-white shadow-lg p-6">
        <h2 className="text-2xl font-bold mb-8" style={{ color: "#062b2d" }}>
          لوحة التحكم
        </h2>
        <nav className="space-y-4">
          {[
            ...
            <div
              ...
              style={{
                backgroundColor:
                  category === tab.key ? PRIMARY_COLOR : "transparent",
              }}
            >
              {tab.icon}
                      ...
                      {newPartner.logo && (
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-green-600">
                            {newPartner.logo.name}
                          </span>
                          <button
                            type="button"
                            onClick={() =>
                              setNewPartner((prev) => ({ ...prev, logo: null }))
                            }
                  ...
                  <div className="p-6">
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div
                        ...
                        } items-center justify-center rounded-lg border-2 border-dashed border-gray-300`}
                      >
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    <label
                      className="flex items-center gap-2 cursor-pointer px-4 py-3 text-white rounded-lg hover:bg-green-700 transition"
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    الصيغ المدعومة: JPG, PNG, WEBP, SVG (الحد الأقصى 5MB)
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {newDoctor.image && (
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-green-600">
                            {newDoctor.image.name}
                          </span>
                          <button
                            type="button"
                            onClick={() =>
                              setNewDoctor((prev) => ({ ...prev, image: null }))
                            }
                            className="text-red-600 hover:text-red-800"
                          >
                            <FaTimes />
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mb-2">
                          {doctor.specialty?.name ||
                            categories.find((c) => c._id === doctor.specialty)
                              ?.name ||
                            "غير محدد"}
                        </p>
                        {doctor.isActive ? (
                ...
                <div className="flex flex-wrap gap-3">
                  {availableYears.map((year) => (
                    <div
                      ...
                      style={{
                        backgroundColor:
                          selectedYear === year ? PRIMARY_COLOR : undefined,
